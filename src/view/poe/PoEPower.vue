<template>
  <div>
    <div class="poe">
      ...
      <div class="form">
        <v-form>
          <v-form-item label="PoE Consumption Power:">{{ruleForm.cousumPower}}W</v-form-item>
          <v-form-item label="PoE Remaining Power:">{{ruleForm.remainingPower}}W</v-form-item>
        </v-form>
      </div>
    </div>
    <div class="config">
    ...
    <div class="editDialog">
      <v-dialog name="dialog2" v-model="showInputDialog" :title="dialogTitle" :close-on-click-modal="closeOnClickModal" :modal="true" @confirm="handleConfirm" @cancel="handleCancel" append-to-body>
        <v-form ref="form" :model="ruleDialogForm">
          <v-form-item label="Select Port" prop="name">
            <v-checkbox-group is-select-all name="check-g2" select-text="All" v-model="checked" :options="options"></v-checkbox-group>
          </v-form-item>
